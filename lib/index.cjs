...
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    webModelService: import_koishi.Schema.object({
      model: import_koishi.Schema.dynamic("model").default("无").description("Web Search 使用的首选 ChatLuna 模型"),
      fallbackModel: import_koishi.Schema.dynamic("model").default("无").description("首选模型失败后重试使用的备选模型")
    }).description("Web 搜索模型配置")
  }),
  import_koishi.Schema.object({
    xModelService: import_koishi.Schema.object({
      model: import_koishi.Schema.dynamic("model").default("无").description("X Search / X Read 使用的首选 Grok 模型"),
      fallbackModel: import_koishi.Schema.dynamic("model").default("无").description("首选模型失败后重试使用的备选 Grok 模型")
    }).description("X 搜索/读取模型配置")
  }),
  import_koishi.Schema.object({
    webSearchTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否注册 Web Search 工具"),
      name: import_koishi.Schema.string().default("web_search").description("工具名称"),
      description: import_koishi.Schema.string().default("调用 AI 搜索能力。").description("工具描述"),
      prompt: import_koishi.Schema.string().role("textarea").default(DEFAULT_WEB_SEARCH_PROMPT).description("Web Search 系统提示")
    }).description("Web Search")
  }),
  import_koishi.Schema.object({
    webReadTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否注册 Web Read 工具"),
      name: import_koishi.Schema.string().default("web_read").description("工具名称"),
      description: import_koishi.Schema.string().default("读取链接，并返回对应的 Markdown 版本。").description("工具描述"),
      prompt: import_koishi.Schema.string().role("textarea").default(DEFAULT_WEB_READ_PROMPT).description("Web Read 系统提示")
    }).description("Web Read")
  }),
  import_koishi.Schema.object({
    xSearchTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否注册 X Search 工具"),
      name: import_koishi.Schema.string().default("x_search").description("工具名称"),
      description: import_koishi.Schema.string().default("调用由 AI 支持的 X 平台搜索能力。").description("工具描述"),
      prompt: import_koishi.Schema.string().role("textarea").default(DEFAULT_X_SEARCH_PROMPT).description("X Search 系统提示")
    }).description("X Search")
  }),
  import_koishi.Schema.object({
    xReadTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否注册 X Read 工具"),
      name: import_koishi.Schema.string().default("x_read").description("工具名称"),
      description: import_koishi.Schema.string().default("读取 X 平台帖子链接，并返回对应的 Markdown 版本。").description("工具描述"),
      prompt: import_koishi.Schema.string().role("textarea").default(DEFAULT_X_READ_PROMPT).description("X Read 系统提示")
    }).description("X Read")
  }),
  import_koishi.Schema.object({
    grokBuiltinTools: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否为 Grok 模型启用下列内置工具"),
      imageUnderstanding: import_koishi.Schema.boolean().default(true).description("是否允许 Grok 查看搜索/读取结果中的图片（虽然 Grok 在 API 中的图像识别能力较差，但对于它的搜索过程我们无法干预，开启后可能在一定程度上会提高它对于搜索结果的总结质量）"),
      webSearchWithXSearch: import_koishi.Schema.boolean().default(true).description("是否允许 Grok 在 Web Search 时进行 X Search（丰富搜索结果）"),
      xVideoUnderstanding: import_koishi.Schema.boolean().default(false).description("是否允许 Grok 查看搜索/读取结果中的视频（仅限 X 平台，使用官方 API 时，可能会消耗更多 token）"),
      xReadImageUnderstanding: import_koishi.Schema.boolean().default(false).description("是否允许 Grok 在 X Read 中查看图片（不建议开启，Grok 在 API 中的图像识别能力较差，会降低返回质量。建议在本插件中配置图像描述服务代替此选项。）")
    }).description("Grok 内置工具配置")
  }),
  import_koishi.Schema.object({
    jinaReader: import_koishi.Schema.object({
      apiKey: import_koishi.Schema.string().role("secret").default("").description("Jina Reader API Key，可选。遇到速率限制时可填写"),
      timeoutSeconds: import_koishi.Schema.number().min(5).max(120).default(60).description("Jina 请求超时（秒）")
    }).description("Jina Reader 配置")
  }),
  import_koishi.Schema.object({
    imageService: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(false).description("是否为搜索/读取结果中的图片 URL 生成描述"),
      ...
      maxImages: import_koishi.Schema.number().min(1).max(20).default(10).description("每次工具调用最多描述的图片数"),
      taskConcurrency: import_koishi.Schema.number().min(1).max(20).default(3).description("图片描述并发数"),
      requestTimeoutSeconds: import_koishi.Schema.number().min(1).max(120).default(20).description("图片下载超时（秒）")
    }).description("图片描述服务")
  }),
  import_koishi.Schema.object({
...
var webSearchSchema = import_zod.z.object({
  query: import_zod.z.string().describe("需要搜索的关键词或问题。")
});
var webReadSchema = import_zod.z.object({
  url: import_zod.z.string().url().describe("需要读取的网页 URL。")
});
var xSearchSchema = import_zod.z.object({
  query: import_zod.z.string().describe("需要在 X/Twitter 中搜索的关键词或问题。")
});
var xReadSchema = import_zod.z.object({
  url: import_zod.z.string().describe("需要读取的 X/Twitter URL，通常是推文、用户主页或搜索结果链接。")
});
...
__name(meta, "meta");
var WebSearchTool = class extends import_tools.StructuredTool {
  constructor(deps) {
    super({});
    this.deps = deps;
...
var WebReadTool = class extends import_tools.StructuredTool {
  constructor(deps) {
    super({});
    this.deps = deps;
...
var XSearchTool = class extends import_tools.StructuredTool {
  constructor(deps) {
    super({});
    this.deps = deps;
...
var XReadTool = class extends import_tools.StructuredTool {
  constructor(deps) {
    super({});
    this.deps = deps;
  ...
  for (const candidate of candidates) {
    try {
      ...
      const msg = await ref.value.invoke(
        [
          new import_messages.SystemMessage(deps.tool.prompt),
          new import_messages.HumanMessage(parts.join("\n"))
        ],
        {
          overrideRequestParams
        }
      );
      return (0, import_string.getMessageContent)(msg.content).trim();
    } catch (err) {
      lastError = err;
...
function buildOverrideRequestParams(deps, configuredModel, chatModel) {
  ...
  if (deps.action === "web_search" && platform === "openai" && lowerModelName.startsWith("gpt-")) {
    return {
...
async function withImages(ctx, cfg, text, ensureImageModelRef) {
  ...
  if (!Array.isArray(ref.value.modelInfo?.capabilities) || !ref.value.modelInfo.capabilities.includes(import_types.ModelCapabilities.ImageInput)) {
    return text;
  }
  const descriptions = {};
  let idx = 0;
      ...
      async () => {
        while (idx < urls.length) {
          ...
          try {
            ...
            const mime = (0, import_string.getImageType)(buf) || "image/jpeg";
            if (mime === "image/gif") {
              continue;
            }
            const msg = await ref.value.invoke([
            ...
            const desc = (0, import_string.getMessageContent)(msg.content).trim();
            if (desc) {
